...
class myAgentP1(CaptureAgent):
  ...
  Phase Number: 1
  Description of Bot: For this problem, we find the max Q_value for each state with its possible
  action pair and choose the action with the largest Q_value.
  To find the value for each (state, aciton) pair, we introduce 4 features
  1. state score for current state
  2. nearest map distance for pacman that I manipulate
  3. total distance of food from me
  4. distance between me and my teammate
  By combining those features, we come up with their weights and finally pass all 10 tests
  """
