    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # first, we need to find the positions that ghosts are most probably at
        ghostPositions = [dist.argMax() for dist in livingGhostPositionDistributions]
        # find the minimum distance for each action
        distance = []
        for action in legal:
            nextPosition = Actions.getSuccessor(pacmanPosition, action)
            temp = [self.distancer.getDistance(nextPosition, ghostPosition) for ghostPosition in ghostPositions]
            distance.append(min(temp))
        # find the action with minimum distance
